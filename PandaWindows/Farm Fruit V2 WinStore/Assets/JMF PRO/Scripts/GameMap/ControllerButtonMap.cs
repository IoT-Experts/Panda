...
public class ControllerButtonMap : MonoBehaviour
{
    ...
    IEnumerator ShowLayerShop()
    {
        ...
        if (GameObject.FindGameObjectWithTag("layershop")==null)
        {
            LayerShop = Instantiate(Resources.Load<GameObject>("Prefabs/Shop/LayerShop")) as GameObject;
            ...
            LayerShop.tag = "layershop";
        }
        else
        {
            LayerShop.GetComponent<Animator>().Play("LayerShopShow");
        }
    ...
    public void ChangeLevel()
    {
        ...
        if (ObscuredPrefs.GetInt("levelmax") != 0)
        {
            ...
            if (level < levelMax + 1)
            {
                Debug.Log("show quảng cáo video");
                admodads.viewadsvideo = true;
            }
        }
    }
