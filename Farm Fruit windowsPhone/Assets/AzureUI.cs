...
{
    //public Text text;
    public GetDataFacebook getDataFacebook;
    List<string> lstId = new List<string>();
    public List<pandafruitfarm> _leaderboardItems = new List<pandafruitfarm>();

    // Use this for initialization   

    // Item to insert
    private pandafruitfarm _leaderboard = new pandafruitfarm()
    {
        Level = "",
        Username = "",
        Score = 0,
    ...
    // Item to update
    public pandafruitfarm _selectedItem = new pandafruitfarm()
    {
        Level = "",
        Username = "",
        Score = 0,
    ...
    void Update()
    {
        if (ManagerDelegate.showDataQuery)
        {
            var list = ManagerDelegate.lstDataAzure;
            for (int i = 0; i < list.Count; i++)
            {
                lstId.Add(list[i].IdFacebook);
                if (string.IsNullOrEmpty(list[i].Username))
                {
                    list[i].Username = "Join";
                }
                if (list[i].Username.Length > 10)
                {
                    list[i].Username = "... " + list[i].Username.Substring(list[i].Username.Length - 10);
                }
                getDataFacebook.lstName[i].text = list[i].Username;
                getDataFacebook.lstScore[i].text = list[i].Score.ToString();
            }
            StartCoroutine(getDataFacebook.getAvatarFriend(lstId));
            ManagerDelegate.showDataQuery = false;
        }
    }

    public void QueryIdFacebook(string idFacebook, string Level,long score)
    {
        _leaderboardItems.Clear();
        ManagerDelegate.parameterAzure = Level;
        ManagerDelegate.scoreAzure = score;
        ManagerDelegate.IdFacebook = idFacebook;
        ManagerDelegate.QueryIdFb();
    }
   
    public void QueryListLevel(string level)
    {
        ManagerDelegate.parameterAzure = level;
        ManagerDelegate.ShowData();    
    }      
}
