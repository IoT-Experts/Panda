...
public class AnimatedSprite_test : MonoBehaviour
{
    ...
    void animateSprite()
    {
        ...
        if (Time.time > animate.lastAnimFrameTime + (1 / (animate.frameRate * (animations[animate.anim].characterInfo[0].uv.width + 1))))
        {
            ...
            if (animate.frame > animations[animate.anim].characterInfo.Length - 2 || (animate.frameStep == -1 && animate.frame < animations[animate.anim].characterInfo[0].uv.y))
            { //if the frame has incremented past the length of the animation, or backwards in the case of pingpong
                switch (animations[animate.anim].characterInfo[0].index)
                { //choose behaviour
    ...
    void changeAnim(anims a, int f)
    { //animation and frame number in
        animate.anim = (int)a;
        animate.TM.font = animations[animate.anim];
        animate.frame = f;
    ...
    void Update()
    {
        ...
        if (Mathf.Abs(buttons.h) > 0.1)
        { //if h input axis buttons are pressed
            buttons.movePressed = true;
            if (movement.currentDirection != new Vector3(buttons.h, 0, 0))
            { //if currentDirection is not the input direction
                if (!movement.crouching)
                { //cant turn on the stop while crouching
        ...
        if (buttons.v > 0.1)
        { //...if jump is pressed...
            if (!movement.jumping && !movement.falling && buttons.jumpReleased)
            { //if not currently in a jump or fall
                buttons.jumpPressed = true;
                buttons.crouchPressed = false;
                buttons.jumpReleased = false;
            }
        }
        else if (buttons.v < -0.1)
        { //...or if crouch is pressed...
        ...
        { //neither jump or crouch pressed
            buttons.crouchPressed = false;
        ...
        if (buttons.movePressed)
        { //if move buttons are pressed
            if (!movement.moving && !movement.crouching)
            { //if not currently moving or crouching
                movement.moving = true; //set moving
                if (!movement.jumping && !movement.falling)
                { //if not mid-jump or fall
                    changeAnim(anims.Run, 0); //play run anim, unless...
                }
            }
            if (buttons.moveChanged)
        ...
        { //h input axis has no input (user stopped pressing)
            if (movement.moving)
            {
        ...
        { //if the character is grounded...
            ...
            else if (buttons.crouchPressed)
            { //...or if crouch is pressed...
                if (!movement.crouching)
                { //...and not currently crouching...
            ...
            { //...otherwise not pressing crouch or jump, while grounded
                if (movement.crouching)
                { //if he was crouching
        ...
        { //character is in the air
            if (Time.time > movement.timeLastGrounded + 0.15)
            { //this stops changing to falling for only small drops eg. stairs
                if (!movement.jumping && !movement.falling)
                { //if its not airborn because of jumping, or falling after a jump, must be falling off a platform
        ...
        if (movement.currentVerticalSpeed < 1)
        { //if the jump nears its apex
            if (movement.jumping)
            { //while jumping
                changeAnim(anims.Fall, 0); //start falling
        ...
        if (movement.moving)
        { //if the character is moving after all input is accounted for
        ...
        { //for 'de-bugging' animation frames hold B to slow-mo
            Time.timeScale = 0.2f;
        }
        else
        {
